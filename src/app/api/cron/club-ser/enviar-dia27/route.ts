...
import { getTemplateDia27, getAsunto, procesarTemplate } from "@/lib/club-ser/templates-email"
...
export async function GET(request: Request) {
  ...
  for (const cupon of cupones || []) {
    ...
    const html = procesarTemplate(getTemplateDia27(), {
      nombre: cliente.nombre, descuento: cupon.valor, codigo: cupon.codigo,
    })

    const ok = await enviarEmail({
      destinatario: cliente.email, nombre: cliente.nombre,
      asunto: getAsunto("dia27", cupon.estado_cliente, cupon.nivel_cliente),
      html, campanaId: campana.id, clienteId: cliente.id, tipo: "dia27",
    })
    if (ok) enviados++
    await new Promise((r) => setTimeout(r, 200))
  }

  await supabase.from("club_ser_campanas").update({ fecha_envio_dia27: new Date().toISOString() }).eq("id", campana.id)
